...
class Setup extends CActiveRecord
{
	...
	public function getbarcodelabelurl($text)
	{
		//return 'http://84.9.30.62:260//test/poppinsappliancestock/ims/images/barcode/barcode.php?codetype=Code25&size=100&text='.$text;
		return 'http://84.9.30.62:260//test/poppinsappliancestock/ims/images/barcode/barcode.php?print=true&size=100&text='.$text;
	...
	public function updatenotesorcomments($text, $model, $attribute)
	{
		...
		$text = trim($text);
		$existingvaluesinmodel = $model->findByPk($model->item_id);
	...
	public function formatinboundoutbounddate($unixdate)
	{

		if ($unixdate!='')
			return date('d-M-y h:i a ', $unixdate );
		else
			return '';
	...
	public function formatdate($unixdate)
	{
		if ($unixdate!='')
	...
	{
		return "YaaIamTheOneYouAreLookingFor";
	}
